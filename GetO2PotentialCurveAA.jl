...
function MakeO2AnionMolecFile()
    ...
    fileID1 = open("O2_ecp_anion_data.txt","w");
    fileID2 = open("O2_fullE_anion_data.txt","w");
...
function EnergyAtDistanceOO_ECP(dist::Real,order::Int)
    ...
    return EnergyAtDistanceOO(dist,order,"O2_ecp_anion_data.txt");
end

function EnergyAtDistanceOO_FullE(dist::Real,order::Int)
    # Returns the energy of the uncorelated model of two molecules of water
    # on the OH reaction coordinate at distance dist.
    return EnergyAtDistanceOO(dist,order,"O2_fullE_anion_data.txt");
end
...
file_name = "Test Data/Gaussian Data/O2_anion/O2_anion.log";
e0 = GetEnergyFromLogFile(file_name);
surf_energy = zeros(Float64,300,3);
react_coord = zeros(Float64,300,3);

for i in 0:299
    base_name = "Test Data/Gaussian Data/AnionAnion/O2 (2S+1=1)/O2_O2_";
...
for i in 0:299
    base_name = "Test Data/Gaussian Data/AnionAnion/O2 (2S+1=3)/O2_O2_";
    local file_name = base_name * string(i) * ".log";
...
# for i in 0:199
#     base_name = "Test Data/Gaussian Data/AnionAnion/O2 (2S+1=5)/O2_O2_";
#     local file_name = base_name * string(i) * ".out";
...
# model1_coord = (collect(0:800) .* (18.0/800)) .+ 1.0;
...
ECP_model1_params1 = GetXCCoeffs_ECP(ECP_order);
ECP_model1_e1 = copy(ECP_all_model1_e[:,1]);
ECP_model1_e1 += ECP_xc_model1*ECP_model1_params1;
...
FullE_model1_params1 = GetXCCoeffs_ECP(FullE_order);
FullE_model1_e1 = copy(FullE_all_model1_e[:,1]);
FullE_model1_e1 += FullE_xc_model1*FullE_model1_params1;

model1_coord .*= a0;

kjmol = 2625.5002;
ECP_model1_e1 *= kjmol;
FullE_model1_e1 *= kjmol;
surf_energy *= kjmol;
...
plot!(model1_coord,ECP_model1_e1,labels="This Work (Full E. fit)",linewidth=l_width);
plot!(model1_coord,FullE_model1_e1,labels="This Work (ECP fit)",linewidth=l_width);

plot!(ylims=[200,800],xlims=[1.75,4.5]);
plot!(yticks=200:200:800);
